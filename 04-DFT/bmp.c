...
double **allocate_double_array(int length, int width)
{
   int i;
   double **the_array;
...
int free_double_array(double **the_array, int length)
{
    int i;
    for(i=0; i<length; i++)
        free(the_array[i]);
...
void flip_image_array(short** the_image, int rows, int cols)
{
    int  i, j;
    int rd2;
    short **temp;
